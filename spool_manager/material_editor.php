...
if ( $_GET["edit"] == "1"){  
  $edit_id = $_GET["id"];
 $sql = $pdo->prepare("select id,name,density from materials where id = :id");
$sql->execute([
':id' => $edit_id
]);
...
$sql = $pdo->prepare("update materials SET name = :name, density = :density where id = :id ");
//$sql->execute();
$sql->execute(array(
":id" => $_POST[id],
":name" => $_POST[name],
...
$sql = $pdo->prepare("insert into materials (name, density)  values(:name, :density) ");
//$sql->execute();
$sql->execute(array(
":name" => $_POST[name],
":density" => $_POST[density],
...
<?php
$sql = $pdo->query('select id,name,density from materials order by name');
while ($row = $sql->fetch()){
echo '<tr class="center">';
echo '<td>'.$row[name].'</td>';	
echo '<td>'.$row[density].'</td>';	
echo '<td><a class="edit_button" href=material_editor.php?edit=1&id='.$row[id].'>редактировать</a></td>';
echo  '</tr>';
}
  echo '</table>';
